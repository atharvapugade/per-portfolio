  ...
  return (
    ...
    >
      <nav className="flex items-center gap-2 sm:justify-between sm:gap-3">
        <a href="#hero" className="group leading-tight">
          <p className="text-[12px] font-semibold text-white transition-colors group-hover:text-indigo-200 sm:text-base">
            Atharv S. Apugade
          </p>
        ...
        <div className="no-scrollbar ml-auto flex max-w-[64%] items-center justify-end gap-1 overflow-x-auto sm:hidden">
          {links.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="shrink-0 rounded-full border border-white/12 bg-white/5 px-2 py-1 text-[10px] font-medium text-slate-200"
            >
              {link.label}
            </a>
          ))}
